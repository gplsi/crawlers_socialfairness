...
class CrawlerMarca(ABC):
    ...
    def parse(url):
        ...
        if response.status_code == 200:
            ...
            if bs:
                sections_head = bs.find_all('h1', {'class': 'ue-c-article__headline js-headline js-headlineb'})
                ...
                sections_date = bs.find_all('div', {'class': 'ue-c-article__publishdate'})
                sections_date = sections_date[0].find_all('time')
                date = sections_date[0].text.strip()

                sections_body = bs.find_all('div', {'class': 'ue-c-article__body'})
                if sections_body:
                    for element in sections_body[0]:
                        if isinstance(element, Tag):
                            if 'p' == element.name:
                                ...
                                if (element.attrs['class'][0] == 'ue-c-article__subheadline'):
                                    paragraphs += element.text + '\n\n'
